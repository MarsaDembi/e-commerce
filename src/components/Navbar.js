        ...
        <nav id="topnav" className="defaultscroll is-sticky w-full z-50 border-b border-gray-200 dark:border-gray-700 bg-white">
            <div className="container mx-auto flex items-center justify-between">
                ...
                >
                    ...
                    <Link href="/contact" className={pathname === '/contact' ? 'text-green-600' : 'hover:text-green-600'}>Contact</Link>
                    <Link href="/product" className={pathname === '/product' ? 'text-green-600' : 'hover:text-green-600'}>Product</Link>
                    {status === "authenticated" && (
                        <>
                            <Link href="/dashboard" className={pathname === '/dashboard' ? 'text-green-600' : 'hover:text-green-600'}>Dashboard</Link>
                            {session?.user?.role === 'admin' ? (
                                <Link href="/admin" className={pathname === '/admin' ? 'text-green-600' : 'hover:text-green-600'}>
                                    Admin
                                </Link>
                            ) : (
                                <Link href="/user" className={pathname === '/user' ? 'text-green-600' : 'hover:text-green-600'}>
                                    User
                                </Link>
                            )}
                        </>
